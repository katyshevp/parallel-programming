...
void help(const char* name, const char* message) {
    std::cout << "\n\n" << std::string(message) +
        "\nThis is the multiplication of sparse complex number matrices application.\n\n" +
        "Please provide arguments in the following format:\n\n" +
        " $ " + name + "<number of columns> <number of complexes in the row>\n" +
        "Where all variables are unsigned integers.\n";
}
...
#pragma omp for private(j, index1, index2, index3, index4)schedule(static, chunk)
    ...
    std::cout << "Runtime: " << t2 - t1 << std::endl;
}
...
int main(int argc, char* argv[]) {
    ...
    if ((argc != 3)) {
        help(argv[0], "ERROR: Wrong number of parameters.");
        return 1;
    }
    ...
    try {
        ...
        help(argv[0], "ERROR: Wrong parameters format.");
        return 1;
    }

    if ((nzInRow > n)) {
        help(argv[0], "Invalid input count of notnull in string");
        return 1;
    }

    std::cout << "Task ¹2. The multiplication of sparse matrices. CRS. Complex type.\n";
